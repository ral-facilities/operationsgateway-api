...
class HDFDataHandler:
    ...
    def _extract_raw_file(
        ...
        value: Any,
    ) -> tuple[RawFileChannelModel, Literal[False]] | tuple[None, list[dict[str, str]]]:
        """
        Extract data for a raw file in the HDF file and place the data into
        relevant Pydantic models.
        """
        relative_path = RawFile.get_relative_path(self.record_id, channel_name)

        if self._unexpected_attribute("raw_file", value):
            internal_failed_channel.append(
                {channel_name: "unexpected group or dataset in channel group"},
            )
            return None, internal_failed_channel
        ...
        try:
            metadata = RawFileChannelMetadataModel(**channel_metadata)
            channel = RawFileChannelModel(
                metadata=metadata,
    ...
    async def extract_channels(self) -> None:
        ...
        for channel_name, value in self.hdf_file.items():
            ...
            elif value.attrs["channel_dtype"] == "raw_file":
                channel, fail = self._extract_raw_file(
                    internal_failed_channel,
                    channel_name,
                    channel_metadata,
                    value,
    ...
    def _update_data(
        ...
        for key in checker_response["rejected_channels"].keys():
            ...
            elif channel.metadata.channel_dtype == "raw_file":
                HDFDataHandler.remove_channel(raw_files, channel.file_path)

            del record_data.channels[key]

        return record_data, images, waveforms, raw_files

    @staticmethod
    def remove_channel(
        models: list[ImageModel] | list[WaveformModel] | list[RawFileModel],
        path: str,
    ) -> None:
