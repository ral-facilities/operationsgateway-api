...
async def submit_hdf(
    ...
    record_data, images, waveforms, raw_files = HDFDataHandler._update_data(
        checker_response,
        record_data,
        images,
        waveforms,
    ...
    log.debug("Processing raw files")
    failed_raw_file_uploads = []
    for raw_file_model in raw_files:
        raw_file = RawFile(raw_file_model)
        failed_upload = raw_file.insert_raw_file()  # Returns channel name if failed
        # if the upload to echo fails, don't process any further
        if failed_upload:
            failed_raw_file_uploads.append(failed_upload)
    ...
    all_failed_upload_channels = (
        failed_waveform_uploads + failed_image_uploads + failed_raw_file_uploads
    )
