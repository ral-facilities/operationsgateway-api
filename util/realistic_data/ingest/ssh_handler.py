...
class SSHHandler:
    ...
    def import_experiments(self) -> None:
        self.connection.run(
            f"mongoimport --db='{Config.config.database.name}' --collection='experiments' --mode='upsert'"
            f" --file='{Config.config.database.remote_experiments_file_path}'",
        )
        self.connection.run(f"rm -f {Config.config.database.remote_experiments_file_path}")
