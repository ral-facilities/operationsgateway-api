...
To combat this issue, the CI now sets up a local instance of `RustFS` in a Docker container which the API connects to when it's launched in the ingestion script. RustFS is a way of having S3 storage locally, mimicking AWS (and therefore Echo S3).
